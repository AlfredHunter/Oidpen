 ...
 * Register for all key events
 */
extern uint8 RegisterForOids( uint8 task_id );

extern uint8 RegisterFor_gSensors( uint8 task);
extern uint8 OnBoard_Send_gSensors( uint8 xyz, int16 value);
...
extern void OnBoard_OidRecvCallback( uint16 oid);
