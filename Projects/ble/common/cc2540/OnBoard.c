...
static uint8 registeredOidTaskID = NO_TASK_ID;
static uint8 registered_gSensorTaskID = NO_TASK_ID;
...
uint8 RegisterForOids( uint8 task)
{
  
  if ( registeredOidTaskID == NO_TASK_ID )
  {
    registeredOidTaskID = task;
    return ( true );
  }
  else
    return ( false );
...
uint8  OnBoard_SendOids( uint16 oid)
{
    oid_t *msgPtr;
    if ( registeredOidTaskID != NO_TASK_ID )
    {
      // Send the address to the task
      msgPtr = (oid_t *)osal_msg_allocate( sizeof(oid_t) );
      if ( msgPtr )
      {
        msgPtr->hdr.event = OID_CHANGE;
        msgPtr->oid = oid;
       
        osal_msg_send( registeredOidTaskID, (uint8 *)msgPtr );
      }
      return ( SUCCESS );
    }
    else
...
uint8 RegisterFor_gSensors( uint8 task)
{
  
  if ( registered_gSensorTaskID == NO_TASK_ID )
  {
    registered_gSensorTaskID = task;
    return ( true );
  }
  else
    return ( false );
...
uint8 OnBoard_Send_gSensors( uint8 xyz, int16 value)
{
    gSensor_t *msgPtr;
    if ( registered_gSensorTaskID != NO_TASK_ID )
    {
      // Send the address to the task
      msgPtr = (gSensor_t *)osal_msg_allocate( sizeof(gSensor_t) );
      if ( msgPtr )
      {
        msgPtr->hdr.event = GENSOR_CHANGE;
        msgPtr->xyz = xyz;
        msgPtr->value = value;
       
        osal_msg_send( registered_gSensorTaskID, (uint8 *)msgPtr );
      }
      return ( SUCCESS );
    }
    else
...
void  OnBoard_OidRecvCallback( uint16 oid )
{
    ...
    if( OID_POWER_OFF == getOidState() ) //¹Ø±ÕOIDºÍ»½ÐÑÊ±ÓÐÊý¾Ý·¢³ö
      return;
