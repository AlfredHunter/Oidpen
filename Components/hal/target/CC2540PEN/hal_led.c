...
void HalLedInit (void)
{
  ...
  HAL_CONFIG_IO_OUTPUT(HAL_LED_B_PORT, HAL_LED_B_PIN, 1);
  HAL_CONFIG_IO_OUTPUT(HAL_LED_R_PORT, HAL_LED_R_PIN, 1);
  HAL_CONFIG_IO_OUTPUT(HAL_LED_G_PORT, HAL_LED_G_PIN, 1);
...
void HalLedEnterSleep( void )
{
  ...
  HalSleepLedState = 0;
  HalSleepLedState |= HAL_STATE_LED_B();
  HalSleepLedState |= HAL_STATE_LED_R();
  HalSleepLedState |= HAL_STATE_LED_G();
...
void HalLedExitSleep( void )
{
  ...
  HalLedOnOff(HalSleepLedState, HAL_LED_MODE_ON); // 20160116 - zhj È¥µôÆÁ±Î

  /* Restart - This takes care BLINKING LEDS */
  HalLedUpdate(); // 20160116 - zhj È¥µôÆÁ±Î
#endif /* HAL_LED */
