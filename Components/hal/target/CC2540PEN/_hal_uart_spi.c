...
static void HalUARTInitSPI(void)
{
 ...
 // halDigioConfig(&pinSpiCS);
  ...
  // - SPI_CLR_RDY_OUT(); // // - zhj ´Ë´¦ÓÃÓÚSPIÐ´£¬É¾³ý
...
static void spiParseRx(void)
{
  while (1)
  {
    ...
    case SPIRX_STATE_FCS:
      ...
      pktFound = TRUE;
#endif
      // SPI_CLR_RDY_OUT(); /* SPI_RDYOut = 1 */ // - zhj ´Ë´¦ÓÃÓÚSPIÐ´£¬É¾³ý
...
void HalUART_IsrSPI(void) {
  ...
  *tmp = UxDBUF;
   ...
   if (spiTxLen == 0)
  {
    ...
    CLEAR_SLEEP_MODE();
    UxDBUF = 0x00;
    // SPI_SET_RDY_OUT(); // - zhj ´Ë´¦ÓÃÓÚSPIÐ´£¬É¾³ý
#endif
